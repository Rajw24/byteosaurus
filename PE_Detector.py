...
ufile = st.file_uploader(label="Upload any exe file", type=['exe'], accept_multiple_files=True)
i=0
if ufile is not None:
    for efile in ufile:
        with st.status('Scanning') as sts:
            with open(f"./temp/temp{i}.exe", "wb") as f:
                f.write(efile.getbuffer())
                # st.success('File uploaded successfully.')
                global detect 
                detect = peCheck.submit(f"./temp/temp{i}.exe", efile.name)
                f.flush()
                f.close()
            if detect == 0:
                sts.update(label="Safe", expanded=True, state="complete")
            else:
                sts.update(label="Malicious", expanded=True, state="complete")
        i+=1 
